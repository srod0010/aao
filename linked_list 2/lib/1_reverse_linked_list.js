...
function reverseLinkedList(linkedList) {
  ...
  if (!linkedList.head.next) return linkedList;
    let listTail = linkedList.tail;
    let listhead = linkedList.head;
    let currentNode = listhead;
    let prevNode;

    for (let i = 0; i < linkedList.length-1;i++) {
      prevNode = linkedList.tail.next;
      linkedList.tail.next = currentNode;
      if (prevNode) currentNode.next = prevNode;
    }
    currentNode.next = null;
    prevNode.next = null;
    linkedList.head = listTail;
    linkedList.tail = listhead;
    return linkedList;
